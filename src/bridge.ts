...
export type BridgeResponse = { type: 'send_message'; room: string; text: string };
const config = loadConfig();
const host = process.env.BRIDGE_HOST || '127.0.0.1';
const port = Number(process.env.BRIDGE_PORT || 18080);
const token = process.env.BRIDGE_TOKEN || '';

function json(res: http.ServerResponse, status: number, body: unknown): void { const payload = JSON.stringify(body); res.writeHead(status, { 'content-type': 'application/json; charset=utf-8' }); res.end(payload); }
function allowedRoom(room: string): boolean { return config.rooms.length === 0 || config.rooms.includes(room); }
function userKey(user: BridgeUser | undefined): string { return String(user?.id ?? user?.name ?? 'unknown'); }
function userName(user: BridgeUser | undefined): string { return user?.name || '알 수 없음'; }
function nowText(ts = Date.now()): string { return new Intl.DateTimeFormat('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: false }).format(new Date(ts)); }

function addChat(room: string, user: BridgeUser | undefined): void {
  const key = userKey(user), name = userName(user), now = new Date().toISOString();
  const current = config.chatStats.find((s: ChatStat) => s.room === room && s.userKey === key);
  if (current) { current.count += 1; current.userName = name; current.lastSeenAt = now; }
  else config.chatStats.push({ room, userKey: key, userName: name, count: 1, firstSeenAt: now, lastSeenAt: now });
}

function addCommandLog(room: string, user: BridgeUser | undefined, command: string, result: string): void {
  const item: CommandLog = { at: new Date().toISOString(), room, userKey: userKey(user), userName: userName(user), command, result };
...
function memberLog(room: string, user: BridgeUser | undefined, type: 'JOIN' | 'LEAVE', timestamp: number): string {
  const key = userKey(user), name = userName(user);
...
function command(room: string, text: string): string | null {
  ...
  if (cmd === '방등록해제') { const i = config.rooms.indexOf(room); if (i < 0) return 'ℹ️ 이 채팅방은 등록되어 있지 않습니다.'; config.rooms.splice(i, 1); saveConfig(config); return '✓ 이 채팅방의 봇 등록을 해제했습니다.'; }
  if (cmd === '봇등록') return 'ℹ️ 봇등록 코드는 Termux 패널에서 생성하세요.';
  if (cmd === '채팅순위') { const rows = config.chatStats.filter(s => s.room === room).sort((a, b) => b.count - a.count).slice(0, 20); return rows.length ? ['📊 채팅 순위', ...rows.map((s, i) => `${i + 1}위 ${s.userName} — ${s.count}회`)].join('\n') : '📊 아직 채팅 기록이 없습니다.'; }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    const timestamp = event.timestamp || Date.now(), responses: BridgeResponse[] = [];
    if (event.type === 'chat') {
      addChat(event.room, event.user);
      if (allowedRoom(event.room)) {
        const text = event.text?.trim() || '';
        const reply = command(event.room, text);
        if (text.startsWith(config.prefix)) addCommandLog(event.room, event.user, text, reply ? 'handled' : 'unknown');
        if (reply) responses.push({ type: 'send_message', room: event.room, text: reply });
      }
    } else if (allowedRoom(event.room)) {
      responses.push({ type: 'send_message', room: event.room, text: memberLog(event.room, event.user, event.type === 'member_join' ? 'JOIN' : 'LEAVE', timestamp) });
    }
    saveConfig(config);
    return json(res, 200, { ok: true, responses, requestId: crypto.randomUUID() });
  } catch (error) { return json(res, 400, { error: error instanceof Error ? error.message : 'bad request' }); }
});
server.listen(port, host, () => { console.log(`[BRIDGE] listening on http://${host}:${port}`); console.log(`[BRIDGE] rooms: ${roomListToString(config.rooms)}`); });
