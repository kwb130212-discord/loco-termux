...
function runtimeStatus(): 'RUNNING' | 'STOPPED' | 'CRASHED' {
  const pid = readPid();
  if (processAlive(pid)) return 'RUNNING';
  return desiredRunning() ? 'CRASHED' : 'STOPPED';
}

function startRuntime(): boolean {
  if (!existsSync(AUTH_PATH)) {
    console.log(`${C.yellow}⚠ 먼저 QR 로그인으로 인증 세션을 만들어야 합니다.${C.reset}`);
    return false;
  }
  const current = readPid();
  if (processAlive(current)) {
    setDesired(true);
    console.log(`${C.green}● OpenChat 런타임이 이미 실행 중입니다. PID=${current}${C.reset}`);
    return true;
  }
  clearPid();
  setDesired(true);
  const logFd = openSync(LOG_PATH, 'a');
  const child = spawn('npm', ['run', 'start:openchat'], {
...
function runtimeInfo() {
  ...
  console.log(`런타임: ${label}`);
  console.log(`PID: ${pid ?? '-'}  자동실행: ${desiredRunning() ? 'ON' : 'OFF'}`);
  console.log(`인증: ${existsSync(AUTH_PATH) ? 'OK' : '없음'}`);
...
function startWatchdog() {
  const timer = setInterval(() => {
    ...
    if (processAlive(readPid())) return;
    console.log(`${C.yellow}[WATCHDOG] OpenChat 런타임이 종료되어 2초 후 재시작합니다.${C.reset}`);
...
async function mainMenu() {
  ...
  console.log(`\n${C.cyan}[ SERVICE ]${C.reset}`);
  console.log(' 11. OpenChat 시작');
  console.log(' 12. OpenChat 정지');
  console.log(' 13. OpenChat 재시작');
  console.log(' 14. OpenChat 상태');
  console.log(' 15. OpenChat 로그 마지막 부분');
  console.log(' 16. 기존 LOCO 브리지 패널');
  console.log('  0. 패널 종료 (서비스는 계속 실행 가능)');
  console.log(`${C.dim}\n※ 런타임은 패널과 분리되어 실행되며, 비정상 종료 시 Watchdog가 자동 재시작합니다.${C.reset}`);
}
