...
// Fast local restore: only non-secret metadata is inspected. Tokens are never
// read into Node. The Python adapter still performs the authoritative check.
...
async function restoreSession(): Promise<void> {
  ...
  // Paint the valid local session immediately; do not make the UI wait for a
  // Python interpreter startup or network operation.
  ...
  try {
    ...
    const result = await new Promise<{ code: number | null; error?: string }>(resolve => {
      const timer = setTimeout(() => { child?.kill('SIGTERM'); resolve({ code: null, error: 'session restore timeout' }); }, 10_000);
    ...
    // A valid cache remains usable during a transient Python startup failure.
    if (!cached) {
      loggedIn = false; sessionId = ''; authStatus = '미인증';
