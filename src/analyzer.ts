...
export type KickResult = {
  ok: boolean;
  reason?: 'ADMIN_ONLY' | 'TARGET_NOT_FOUND' | 'UNSUPPORTED_TRANSPORT';
  action?: 'KICK_REQUEST';
  roomId?: string;
  targetUserId?: string;
 ...
 *
 * This class is deliberately transport/protocol agnostic. It stores observed
 * room state and diagnostics and provides a local MOCK session for UI testing.
 * The MOCK session is never a Kakao credential and is never persisted.
 */
export class RoomAnalyzer {
  ...
  private appendEvent(event: MemberEvent): void {
  ...
  private save(): void {
    ...
    try {
      ...
      console.error('[Analyzer] save failed:', error instanceof Error ? error.message : error);
    }
  }
  ...
  private load(): void {
    ...
    try {
      ...
      if (Array.isArray(raw.events)) this.events.push(...raw.events.filter(Boolean).map((x: any) => ({
        roomId: String(x.roomId ?? x.room_id ?? ''),
        userId: String(x.userId ?? x.user_id ?? ''),
        nickname: String(x.nickname ?? '알 수 없음'),
        event: this.VALID_EVENTS.has(x.event) ? x.event : 'READ',
        at: String(x.at ?? this.now()),
        count: Number.isFinite(Number(x.count)) ? Math.max(0, Math.floor(Number(x.count))) : 0,
        ...(x.messageId != null || x.message_id != null ? { messageId: String(x.messageId ?? x.message_id) } : {}),
      } as MemberEvent)));

      if (raw.pendingLeaves && typeof raw.pendingLeaves === 'object') {
        for (const [key, value] of Object.entries(raw.pendingLeaves)) {
          const x: any = value;
          if (!x) continue;
          this.pendingLeaves.set(key, {
            ...
            userId: String(x.userId ?? x.user_id ?? ''),
            nickname: String(x.nickname ?? '알 수 없음'),
            leftAt: String(x.leftAt ?? x.left_at ?? this.now()),
            ...(x.messageId != null || x.message_id != null ? { messageId: String(x.messageId ?? x.message_id) } : {}),
          });
        }
      }
      ...
      if (raw.reads && typeof raw.reads === 'object') {
        for (const [messageId, value] of Object.entries(raw.reads)) {
          if (!value || typeof value !== 'object') continue;
          this.reads.set(String(messageId), new Map(Object.entries(value).map(([id, name]) => [String(id), String(name)])));
        }
      }

      if (raw.joinCounts && typeof raw.joinCounts === 'object') {
        for (const [key, value] of Object.entries(raw.joinCounts)) this.joinCounts.set(String(key), Number(value) || 0);
      }

      if (raw.online && typeof raw.online === 'object') {
        for (const [key, value] of Object.entries(raw.online)) {
          const x: any = value;
          if (!x || typeof x !== 'object') continue;
          this.online.set(key, { userId: String(x.userId ?? x.user_id ?? ''), nickname: String(x.nickname ?? '알 수 없음') });
        }
      }

      if (Array.isArray(raw.sessionDiagnostics)) {
        this.diagnostics.push(...raw.sessionDiagnostics.map((x: any) => ({
          userId: String(x.userId ?? x.user_id ?? ''),
          ...(x.sessionId != null || x.session_id != null ? { sessionId: String(x.sessionId ?? x.session_id) } : {}),
          observedAt: String(x.observedAt ?? x.observed_at ?? this.now()),
          status: String(x.status ?? 'UNKNOWN'),
          ...(x.errorCode != null || x.error_code != null ? { errorCode: String(x.errorCode ?? x.error_code) } : {}),
          ...(x.detail != null ? { detail: String(x.detail) } : {}),
        })));
      }
  ...
  mockLogin(userId: string, nickname: string, roomId = 'local', forceSuccess = true): MockSession {
    ...
    const name = String(nickname).trim() || '알 수 없음';
    const room = String(roomId).trim() || 'local';
    ...
    const session: MockSession = {
      userId: id,
      nickname: name,
      sessionId: `mock_${crypto.randomUUID().replace(/-/g, '').slice(0, 12)}`,
      createdAt,
      mode: 'MOCK',
      authenticated: true,
    };
    ...
    const key = this.memberKey(room, id);
    const count = (this.joinCounts.get(key) ?? 0) + 1;
    this.joinCounts.set(key, count);
    this.online.set(key, { userId: id, nickname: name });
  ...
  mockLogout(sessionId: string): boolean {
    const session = this.mockSessions.get(String(sessionId));
  ...
  userJoined(roomId: string, user: any, messageId?: string): number {
    ...
    const { userId, nickname } = this.safeUser(user);
    const key = this.memberKey(room, userId);
    const count = (this.joinCounts.get(key) ?? 0) + 1;
    this.joinCounts.set(key, count);
    this.online.set(key, { userId, nickname });
    this.pendingLeaves.delete(key);
    this.appendEvent({ roomId: room, userId, nickname, event: 'JOIN', at: this.now(), count, ...(messageId ? { messageId: String(messageId) } : {}) });
    this.save();
    return count;
  }

  userLeft(roomId: string, user: any, messageId?: string): string {
    const room = String(roomId);
    const { userId, nickname } = this.safeUser(user);
    const leftAt = this.now();
    const key = this.memberKey(room, userId);
    const record: PendingLeave = { roomId: room, userId, nickname, leftAt, ...(messageId ? { messageId: String(messageId) } : {}) };
    this.pendingLeaves.set(key, record);
    this.online.delete(key);
    this.appendEvent({ roomId: room, userId, nickname, event: 'LEAVE', at: leftAt, count: 0, ...(messageId ? { messageId: String(messageId) } : {}) });
    this.save();
    return this.leaveMessage(nickname, leftAt);
  }

  recordRead(messageId: string, user: any): void {
    const { userId, nickname } = this.safeUser(user);
    const id = String(messageId);
    const bucket = this.reads.get(id) ?? new Map<string, string>();
    bucket.set(userId, nickname);
    this.reads.set(id, bucket);
    this.appendEvent({ roomId: 'unknown', userId, nickname, event: 'READ', at: this.now(), count: 0, messageId: id });
    while (this.reads.size > this.maxReads) this.reads.delete(this.reads.keys().next().value as string);
    this.save();
  }
  ...
  diagnose999(userId: string): { userId: string; observations: number; lastStatus: string | null; lastErrorCode: string | null; lastSessionId: string | null; recommendation: string } {
    const matches = this.diagnostics.filter(x => x.userId === String(userId));
    ...
    return {
      ...
      recommendation: 'Compare the real authentication response and session lifecycle. This analyzer cannot repair a server-side session or bypass authentication.',
    };
  }
  ...
  findLeaveByMessage(roomId: string, messageId: string): PendingLeave | undefined {
    for (const record of this.pendingLeaves.values()) if (record.roomId === String(roomId) && record.messageId === String(messageId)) return record;
    return undefined;
  }
  ...
  validateKick(actorUserId: string, roomId: string, targetUserId: string): KickResult {
    ...
    const target = this.findLeave(roomId, targetUserId);
  ...
  getOnline(roomId: string): Array<{ userId: string; nickname: string }> {
    const prefix = `${String(roomId)}\x1f`;
    return [...this.online.entries()].filter(([key]) => key.startsWith(prefix)).map(([, value]) => ({ ...value }));
  }

  roomEvents(roomId: string, limit = 100): MemberEvent[] {
    return this.events.filter(x => x.roomId === String(roomId)).slice(-Math.max(0, Math.floor(limit)));
  }

  stats(roomId?: string): { events: number; joins: number; leaves: number; reads: number; online: number; diagnostics: number } {
    const events = roomId == null ? this.events : this.events.filter(x => x.roomId === String(roomId));
    return {
      events: events.length,
      joins: events.filter(x => x.event === 'JOIN').length,
      leaves: events.filter(x => x.event === 'LEAVE').length,
      reads: this.reads.size,
      online: roomId == null ? this.online.size : this.getOnline(roomId).length,
      diagnostics: this.diagnostics.length,
    };
  }

  leaveMessage(nickname: string, leftAt: string): string {
    let time = leftAt;
    try { time = new Intl.DateTimeFormat('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: false }).format(new Date(leftAt)); } catch { /* keep ISO */ }
    return [
      `${nickname}님이 나가셨습니다.`,
      '', '[전체보기]', '',
      `${nickname} 님이 ${time}에 나가셨습니다.`,
