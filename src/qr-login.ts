...
export async function loginLocoByQR(options: { deviceUuid?: string; deviceName?: string; modelName?: string; forced?: boolean; checkAllowlist?: boolean; enforceAllowlist?: boolean; appVer?: string } = {}): Promise<LocoQrLoginResult> {
  ...
  console.log(`${T.dim}• QR 생성 전 비필수 allowlist 조회는 생략합니다.${T.reset}\n`);
  const payload = await createAuthByQR({
    ...
    checkAllowlist: options.checkAllowlist ?? false,
...
async function main(): Promise<void> {
  try {
    ...
    if (/이용|횟수|overused|too many|rate|limit/i.test(message)) console.error(`${T.yellow}[QR] 서버 사용 제한으로 보입니다. QR을 연속 생성하지 말고 잠시 후 다시 시도하세요.${T.reset}`);
    process.exitCode = 1;
  }
}
